...
class CFraction(Complex):
	...
	def __init__(self, real=0, imag=0):
		...
		if isinstance(real, Complex) and imag == 0:
			self._real = _Fraction(real.real)
			self._imag = _Fraction(real.imag)
		elif isinstance(real, str) and imag == 0:
			real, imag = self._splitStr(real)
			self._real = _Fraction(real)
			self._imag = _Fraction(imag)
		elif isinstance(real, str) and isinstance(imag, str):
			real = self._splitStr(real)[0]
			imag = self._splitStr(imag)[1]
			self._real = _Fraction(real)
			self._imag = _Fraction(imag)
		elif isinstance(real, Iterable) and len(real) == 2 and isinstance(imag, Iterable) and len(imag) == 2:
			# Support passing tuples as (numerator,denominator) pairs
			self._real = _Fraction(*real)
			self._imag = _Fraction(*imag)
		elif isinstance(real, (int, float, Fraction)) and isinstance(imag, (int, float, Fraction)):
			self._real = _Fraction(real)
			self._imag = _Fraction(imag)
		else:
			raise TypeError(f"can't convert implicitly {type(real)}{f'/{type(imag)}' if imag!=0 else ''} to CFraction")
...
# Methods
	...
	def _splitStr(self, s):
		"""Split string in real and imaginary component"""
		pattern = re.compile(r"^(?=[ij.\d+-])(?P<real>[+-]?(?:\d+(?:\.|\/\d*)?|\.\d+)(?![ij.\d]))?(?P<imag>[+-]?(?:(?:\d+(?:\.|\/\d*)?|\.\d+))?[ij])?$")
		imagC1 = re.compile(r"^([+-])?[ij]$")
		...
		if ret is not None:
			...
			raw_imag = ret.group('imag')

			if raw_imag is not None:
				if imagC1.search(raw_imag) is not None:
					imag = re.sub(imagC1, r'\g<1>1', raw_imag)
				else:
					imag = re.sub(r"^(.+)[ij]$", r'\g<1>', raw_imag)

		real = real if real is not None else 0
		imag = imag if imag is not None else 0

		return real, imag

		# ^(?=[ij.\d+-])(?P<real>[+-]?(?:\d+(?:\.|\/\d*)?|\.\d+)(?![ij.\d]))?(?P<imag>[+-]?(?:(?:\d+(?:\.|\/\d*)?|\.\d+))?[ij])?$
